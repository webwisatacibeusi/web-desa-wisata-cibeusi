...
export const getStaticPaths = async () => {
    ...
    const paths = data.data.map(post => ({
        params: {slug: `${post.Detail_Wisata.Slug}`},
    }));
    return {
        paths: paths,
        fallback: 'blocking',
...
export const getStaticProps = async ({params}) => {
    const res = await fetch(
        `https://www.production-vohbr3y-blyxbfm5ri3hc.us-2.platformsh.site/api/slugify/slugs/detail-wisata/${params.slug}?populate[0]=Wisata&populate[1]=Wisata.Gambar`,
        {
            method: 'GET',
        }
    );
...
export default function CurugPandawa({wisata}) {
    ...
    return (
        <>
            <Meta title={wisata.Nama} description={wisata.Nama}/>
            <Navbar/>
            <Container maxW={'container.xl'}>
                <VStack alignItems={'start'} w={'full'} rowGap={'6'}>
                    <Heading>{wisata.Nama}</Heading>
                    <img
                        src={wisata.Wisata.Gambar[0].url}
                        alt={wisata.Nama}
